"""
03_complexity_ml.py  (Natural Image)
======================================
ML classification using COMPLEXITY features from Natural Image ERG (No Age/Sex).

Features: nAUC (MSE AUC), LRS (Linear Regression Slope) — [No Age/Sex]
Protocol: 5-fold subject-disjoint CV, best classifier by AUC.

Outputs
-------
  results/figures/03_ni_complexity_comparison.png
  results/tables/03_ni_complexity_results.csv

Usage: python src/03_complexity_ml.py
"""
...
COMP_FEATS = ['nAUC_15','nAUC_30','nAUC_45','nAUC_all',
...
if nan_counts.sum() > 0:
    ...
    # Dropping rows missing labels if any
...
# Match original script's shuffle for stability
...
pd.DataFrame(rows).to_csv(os.path.join(TAB_DIR, '03_ni_complexity_results.csv'), index=False)

BG,C='#F7F9FC','#2E7D32'; res=best_overall_results
fig=plt.figure(figsize=(16,6),facecolor=BG)
fig.suptitle(f'Complexity Features — 5-Fold Subject-Disjoint CV · Best: {best_overall_name}',fontsize=14,fontweight='bold')
...
ax_roc.plot([0,1],[0,1],'--',color='#B0BEC5',lw=1.5); ax_roc.plot(fpr,tpr,color=C,lw=2.8,label=f'AUC={res["auc"]:.3f}')
ax_roc.legend(); ax_roc.set_facecolor('white'); ax_roc.grid(alpha=0.2)
...
ax_bar.set_ylim(0,1.2); ax_bar.set_title('Performance Metrics',fontweight='bold'); ax_bar.set_facecolor('white')
...
for ri in range(2):
    for ci in range(2):
        ...
        ax_cm.add_patch(FancyBboxPatch((ci-0.45,ri-0.40),0.90,0.80,boxstyle='round,pad=0,rounding_size=0.08',
...
plt.savefig(os.path.join(FIG_DIR,'03_ni_complexity_comparison.png'),dpi=300,bbox_inches='tight',facecolor=BG)
plt.close()
print(f"✓ Figure → {FIG_DIR}/03_ni_complexity_comparison.png")
...
fig.suptitle('Natural Image — Complexity Features: Model Selection & Feature Importance\n'
             '(5-fold subject-disjoint CV, No Age)', fontsize=12, fontweight='bold')
plt.tight_layout()
_out_sel = os.path.join(FIG_DIR, '03_ni_complexity_model_selection.png')
