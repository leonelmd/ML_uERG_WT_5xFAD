"""
06_handcrafted_ml.py
====================
Machine-learning classification using HAND-CRAFTED ERG features (No Age).

Features used
-------------
  Flash     : Flash_Peak_Max, Flash_Peak_Min, Flash_Peak_P2P, Flash_RMS
  ChirpFreq : ChirpFreq_RMS, ChirpFreq_Std
  ChirpAmp  : ChirpAmp_RMS, ChirpAmp_Max, ChirpAmp_P2P
  Power     : Power_Total, Power_Low, Power_Mid, Power_High

Protocol
--------
* 5-fold subject-disjoint CV
* Multiple classifiers evaluated; best by AUC selected and reported.
* 1,000-iteration bootstrap for confidence intervals.

Outputs
-------
  results/figures/06_handcrafted_roc.png
  results/figures/06_handcrafted_cm.png
  results/tables/06_handcrafted_results.csv

Usage (from chirp_analysis/ folder)
-------------------------------------
    python src/06_handcrafted_ml.py
"""
...
DATA_CSV = os.path.join(ROOT, 'data', 'chirp_hand_crafted.csv')
...
CLF_DEFS = {
    ...
    'k-NN (k=5)':       SVC(kernel='rbf', C=1., probability=True),   # placeholder, see below
}
# Override k-NN properly
from sklearn.neighbors import KNeighborsClassifier
CLF_DEFS['k-NN (k=5)'] = KNeighborsClassifier(n_neighbors=5)
...
nan_counts = df[HAND_FEATS].isna().sum()
if nan_counts.sum() > 0:
...
# Match project standard: shuffle for stable internal calibration splits in CV
...
print(f"\n→ Best classifier: {best_overall_name}  (Pooled AUC={best_overall_auc:.3f})")

# Add bootstrap stats on the aggregated predictions for the bar charts
...
pd.DataFrame(rows).to_csv(
    os.path.join(TAB_DIR, '06_handcrafted_results.csv'), index=False)

# ── Figure ─────────────────────────────────────────────────────────────────────
BG, C = '#F7F9FC', '#1565C0'

fig = plt.figure(figsize=(16, 6), facecolor=BG)
fig.suptitle(f'Hand-Crafted ERG Features — 5-Fold Subject-Disjoint CV · Best: {best_overall_name}',
...
means = [res[k] for k in MKEYS]
errs  = [res['stats'][k][1] for k in MKEYS]
...
plt.savefig(os.path.join(FIG_DIR, '06_handcrafted_comparison.png'),
            dpi=300, bbox_inches='tight', facecolor=BG)
plt.close()

print(f"\n✓ Figure → {FIG_DIR}/06_handcrafted_comparison.png")
print(f"✓ Table  → {TAB_DIR}/06_handcrafted_results.csv")

# ── Summary ────────────────────────────────────────────────────────────────────
s = best_overall_results['stats']
print(f"\nFINAL — Hand-Crafted ML ({best_overall_name}) — 5-Fold CV")
print(f"  Accuracy   : {best_overall_results['acc']:.1%}  (bootstrap ± {s['acc'][1]:.1%})")
print(f"  AUC        : {best_overall_results['auc']:.3f}  (bootstrap ± {s['auc'][1]:.3f})")
print(f"  F1-Score   : {best_overall_results['f1']:.1%}  (bootstrap ± {s['f1'][1]:.1%})")
print(f"  Sensitivity: {best_overall_results['sens']:.1%}")
print(f"  Specificity: {best_overall_results['spec']:.1%}")
...
# Re-train best pipeline on full dataset for stable feature importance
...
fig.suptitle('Chirp — Hand-crafted Features: Model Selection & Feature Importance\n'
             '(5-fold subject-disjoint CV, No Age)', fontsize=12, fontweight='bold')
plt.tight_layout()
_out_sel = os.path.join(FIG_DIR, '06_handcrafted_model_selection.png')
