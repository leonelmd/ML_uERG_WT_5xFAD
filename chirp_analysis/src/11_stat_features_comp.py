"""
11_stat_features_comp.py
=======================
Statistical analysis of Chirp Complexity features across 4 groups.
Normality-based test selection (Parametric vs Non-Parametric).

Outputs:
- results/figures/11_chirp_comp_stats_distribution.png
- results/tables/11_chirp_comp_stats.csv
"""
...
df_feats = pd.read_csv(os.path.join(DATA_DIR, 'complexity_features.csv'))
df = pd.merge(df_meta, df_feats, on='Subject')

GROUP_ORDER = ['WT young', 'WT adult', '5xFAD young', '5xFAD adult']
FEATURES = ['nAUC_all', 'LRS_all', 'nAUC_30', 'LRS_30', 'nAUC_45', 'LRS_45']
...
pd.DataFrame(stat_results).to_csv(os.path.join(TAB_DIR, '11_chirp_comp_stats.csv'), index=False)

# ── Visualization ─────────────────────────────────────────────────────────────
sns.set_theme(style="whitegrid", palette="muted")
COLORS = ['#2563EB', '#1D4ED8', '#E11D48', '#BE123C']

fig, axes = plt.subplots(2, 4, figsize=(28, 18), facecolor='#F8FAFC')
axes = axes.flatten()
...
fig.suptitle('Chirp Complexity Features: Statistical Survey', 
...
fig_out = os.path.join(FIG_DIR, '11_chirp_comp_stats_distribution.png')
